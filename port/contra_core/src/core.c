...
static uint8_t contra_render_cpu_sprite(ContraCore *core, size_t sprite_index, uint8_t sprite_order)
{
...
{
    size_t sprite_index = 26u;
    uint8_t sprite_order = 0u;

    while (sprite_index-- != 0u)
    {
        sprite_order = contra_render_cpu_sprite(core, sprite_index, sprite_order);
    }
}

static void contra_render_intro_background(ContraCore *core)
{
    const uint8_t base_nametable = (uint8_t)(core->ram[CONTRA_RAM_PPUCTRL_SETTINGS] & 0x01u);
    const uint8_t fine_scroll_x = core->ram[CONTRA_RAM_HORIZONTAL_SCROLL];
...
static void contra_render_horizontal_level_background_scrolled(ContraCore *core)
{
    ...
    const uint8_t scroll_pixels = ram[CONTRA_RAM_LEVEL_SCREEN_SCROLL_OFFSET];
    ...
    contra_decode_level_screen_supertiles(core, ram[CONTRA_RAM_LEVEL_SCREEN_NUMBER], current_screen_supertiles, 0u);
    contra_decode_level_screen_supertiles(core, (uint8_t)(ram[CONTRA_RAM_LEVEL_SCREEN_NUMBER] + 1u), next_screen_supertiles, 0u);
...
static void contra_zero_out_nametables(ContraCore *core)
{
    ...
    memset(core->level_screen_supertiles, 0, sizeof(core->level_screen_supertiles));
}
...
static int contra_find_player_bullet_slot(const ContraCore *core, uint8_t player_index, unsigned bullet_limit)
{
    ...
    for (bullet_index = 0u; bullet_index < CONTRA_PLAYER_BULLET_COUNT; ++bullet_index)
    {
        if ((core->ram[CONTRA_RAM_PLAYER_BULLET_SLOT + bullet_index] != 0u) &&
            (core->ram[CONTRA_RAM_PLAYER_BULLET_OWNER + bullet_index] == player_index))
        {
            ++active_bullets;
        }
    }
...
static void contra_set_player_landing_y_offset(ContraCore *core, uint8_t player_index)
{
    ...
    {
        const uint8_t vertical_offset = (uint8_t)((ram[CONTRA_RAM_VERTICAL_SCROLL] & 0x0Fu) | 0xF0u);
        uint8_t landing_y = (uint8_t)(ram[CONTRA_RAM_SPRITE_Y_POS + player_index] + vertical_offset);

        landing_y &= 0xF0u;
        landing_y = (uint8_t)(landing_y - vertical_offset);
        landing_y = (uint8_t)(landing_y + 0x04u);
        ram[CONTRA_RAM_SPRITE_Y_POS + player_index] = landing_y;
    }
}
...
)
{
    ...
    for (projectile_index = 0u; projectile_index < CONTRA_NATIVE_MAX_ENEMY_PROJECTILES; ++projectile_index)
    {
        ...
        projectile->sprite_code = 0x1Eu;
        projectile->sprite_attr = 0x03u;
...
)
{
    ...
    return contra_try_spawn_level_1_enemy_projectile(core, spawn_x, spawn_y, vx, vy, enemy->type);
}

static void contra_update_native_level_1_enemy_projectiles(ContraCore *core)
{
    size_t projectile_index;

    if (!contra_is_native_level_1_active(core))
    {
        return;
    }
...
static void contra_check_native_level_1_player_collisions(ContraCore *core)
{
    uint8_t player_index;
                    ...
                    if (enemy->cooldown == 0u)
                    {
                        ...
                        if (section != 0u)
                        {
                            contra_level_1_bridge_set_overlay(
                                enemy,
                                (unsigned)(section - 1u),
                                contra_level_1_bridge_prev_overlay_tbl[section]
                            );
                        }
                    }
                    else if (enemy->cooldown == 1u)
                    {
                        const uint8_t section = (uint8_t)(enemy->flags & 0x03u);
                        ...
                        contra_level_1_bridge_set_overlay(
                            enemy,
                            section,
                            contra_level_1_bridge_curr_overlay_tbl[section]
                        );
                    }
        ...
        do
        {
            ...
            for (enemy_index = 0u; enemy_index < CONTRA_NATIVE_MAX_ENEMIES; ++enemy_index)
            {
                ...
                switch (enemy_type)
                {
                    ...
                    case 0x10u:
                        enemy->sprite_code = (uint8_t)(((enemy->attrs & 0x01u) != 0u) ? 0x29u : 0x26u);
                        enemy->hp = 0x08u;
                        break;
...
static void contra_load_palettes_color_to_cpu(ContraCore *core, uint8_t num_colors)
{
    ...
    while (write_offset < num_colors)
    {
        ...
        core->ppu_palette[write_offset++] = 0x0Fu;
        ...
        for (color_index = 0u; (color_index < 3u) && (write_offset < num_colors); ++color_index)
        {
            ...
            core->ppu_palette[write_offset++] = color;
        }
...
static void contra_simulate_demo_input(ContraCore *core)
{
    ...
    for (player_index = 1; player_index >= 0; --player_index)
    {
        ...
        if (ram[CONTRA_RAM_DEMO_FIRE_DELAY_TIMER] >= 0x50u)
        {
            ...
            if (auto_fire || ((ram[CONTRA_RAM_FRAME_COUNTER] & 0x07u) == 0u))
            {
...
)
{
    const uint16_t supertile_addr = (uint16_t)(
        contra_level_1_nametable_update_supertile_data_addr + ((uint16_t)supertile_index * 16u)
    );
    const uint8_t supertile_palette = contra_rom_read_u8(
        3u,
        (uint16_t)(contra_level_1_nametable_update_palette_data_addr + supertile_index)
    );
    unsigned tile_y;
...
static void contra_render_frame(ContraCore *core)
{
    const uint8_t *const ram = core->ram;
    ...
    contra_render_native_level_1_enemies(core);
    contra_render_cpu_sprites(core);
}
