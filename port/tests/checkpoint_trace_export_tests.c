...
static const ExpectedTraceRow expected_rows[] = {
    ...
    {"attract_level2_demo", "level2-attract-first-room", 3008u, 1u, 1u, 0u, 0u, 0u, 0u},
    {"attract_level2_demo", "level2-wall-core-loaded", 3009u, 1u, 1u, 0u, 0u, 0u, 1u},
    ...
    {"level2_room_chain", "level2-boss-defeated", 1529u, 1u, 128u, 5u, 0u, 0u, 1u}
};
...
static bool validate_line(const char *line, const ExpectedTraceRow *expected, unsigned line_number)
{
    ...
    for (index = 0u; index < (sizeof(hash_fields) / sizeof(hash_fields[0])); ++index)
    {
        ...
        {
            printf("FAIL trace row %u missing valid %s\n", line_number, hash_fields[index]);
            valid = false;
        }
    }
...
int main(int argc, char **argv)
{
    ...
    if (file == NULL)
    {
        printf("FAIL could not open checkpoint trace %s\n", argv[1]);
        return 1;
    }
    ...
    while (fgets(line, sizeof(line), file) != NULL)
    {
        ...
        {
            printf("FAIL trace has extra row %u\n", line_number + 1u);
            ++failures;
            break;
        }
    ...
    if (line_number != (sizeof(expected_rows) / sizeof(expected_rows[0])))
    {
        printf("FAIL trace row count expected=%u actual=%u\n",
    ...
    printf("PASS checkpoint trace export rows=%u\n", line_number);
    return 0;
}
